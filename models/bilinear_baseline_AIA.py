...
class _DenseAtten(nn.Module):
    def __init__(self, in_channels):
        super(_DenseAtten, self).__init__()

        # self.propose_parts = propose_parts
        self.out_channels = in_channels // 8
        self.conv_f = _dim_change(in_channels, self.out_channels)
        self.conv_g = _dim_change(in_channels, self.out_channels)
        self.conv_h = _dim_change(in_channels, self.out_channels)

        self.W = _dim_change(self.out_channels, in_channels)
        # self.gamma = nn.Parameter(torch.zeros([1]))
        
    def forward(self, x):
        batches = x.size(0)
        # foreground_map = self.foregroundAtten(x)
        g_out = self.conv_g(x)
        f_out = self.conv_f(x)
        h_out = self.conv_h(x)  # NxChannel/2x8x8

        [row_f, col_f] = f_out.size()[-2:]
        [row_g, col_g] = g_out.size()[-2:]

        g_out = g_out.view(batches, self.out_channels, -1)
        f_out = f_out.view(batches, self.out_channels, -1)
        h_out = h_out.view(batches, self.out_channels, -1)  # NxChannel/2x64

        g_out = g_out.permute(0, 2, 1)
        h_out = h_out.permute(0, 2, 1)

        atten_maps_batch_list = []
        self_atten_features_batch_list = []

        atten_maps = torch.matmul(g_out, f_out)  # Nx64(g)x64(f)
        # atten_maps = F.relu(atten_maps)  # Nx64(g)x64(f)
        
        # atten_maps = F.softmax(atten_maps, dim=2)  # Nx64(g)x64(f)
        atten_maps = atten_maps / atten_maps.size(-1) # Nx64(g)x64(f)
        # atten_maps = foreground_map.view(batches, 1, -1) * atten_maps
        self_atten_features = torch.matmul(atten_maps, h_out)  # Nx64(g)xChannel/2

        # atten_maps = atten_maps.contiguous()
        self_atten_features = self_atten_features.permute(0, 2, 1).contiguous() # NxChannel/2x64(g)

        atten_maps = atten_maps.view(batches, -1, row_f, col_f)  # Nx64(g)x8x8
        self_atten_features = self_atten_features.view(batches, -1, row_g, col_g)  # NxChannel/2x8x8

        features_out = x + self.W(self_atten_features)  # NxChannelx8x8

        return atten_maps, features_out

        # if self.propose_parts:
        #     theta_list, trans_param = self.g_predictor(x)
        #     g_loc = self.get_g_loc(x, trans_param)

        #     features_out_list = []
        #     for theta in theta_list:
        #         grid = F.affine_grid(theta, size=torch.Size(features_out_temp.size()))
        #         features_out = F.grid_sample(features_out_temp, grid)  # Nx128x8x8
        #         features_out_list.append(features_out)

        #     return atten_maps, features_out_list, g_loc, theta_list
        # else:
        #     # atten_maps, features_out = self.selfAtten(x, g_out, f_out, h_out, theta=())
        #     features_out = features_out_temp
        #     return atten_maps, features_out

# class DimReduceModule(nn.Sequential):
#     def __init__(self, inChannels, outChannels):
#         super(DimReduceModule, self).__init__()
#         self.add_module('conv', nn.Conv2d(in_channels=inChannels,
#                                           out_channels=outChannels, kernel_size=1, stride=1, padding=0))
#         self.add_module('norm', nn.BatchNorm2d(outChannels))
...
class BilinearAttention(nn.Module):
    def __init__(self, inChannels, outChannels):
        ...
        midChannels = inChannels // 16

        #self.soAtt = SoAtten(midChannels)

        #self.soAtt2 = SoAtten(outChannels)

        self.dimReduceLayer1 = _dim_change(inChannels, midChannels)
        self.dimReduceLayer2 = _dim_change(midChannels*(midChannels+1)//2, outChannels)
        self.dimReduceLayer3 = _dim_change(midChannels*(midChannels+1)//2, outChannels)
    ...
    def forward(self, x):
        ...
        # x = x + F.sigmoid(x) * x        # what to be pooled?
        # x = F.sigmoid(x) * x
        ...
        for i in range(channels):
            ...
            x_list.append(x_temp)
            #else:
                #x_list.append(torch.sqrt(torch.tensor([2], dtype = torch.float)).cuda()*x_temp)

        x = torch.cat(x_list, dim=1) #BRowColxC'
        x = x.view(batches, rows, cols, -1) #BxRowxColxC'
        x = x.permute(0, 3, 1, 2).contiguous() #BxC'xRowxCol

        #mSpatial = x.mean(1, keepdim=True)  #spatial att #TODO: conv layer
        #mx = F.sigmoid(mSpatial) * x
        #mx = mSpatial * x

        mCha = F.avg_pool2d(x, x.size()[2:])  #channel att
        mCha = self.dimReduceLayer3(mCha)
        #mCha = F.softmax(mCha, dim = 1) 
        
        #output = F.sigmoid(mCha) * x
        #output = mCha * x

        output = self.dimReduceLayer2(x)

        output = mCha * output   #No Res setting
...
class Features(nn.Module):
    def __init__(self, height=256, width=128, nchannels=(480, 832, 1024), num_focused_parts=4, drop_rate=0.1, factor_of_scale_factors=1,
        ...
        #self.part_predictor1 = _part_detector(nchannels[0], num_focused_parts, height // (2 ** 3), 
        #                                    width // (2 ** 3), factor_of_scale_factors = 1)

        self.bilinearatt_global = BilinearAttention(nchannels[0], nchannels[0])
        # ============== Block 2 ==============
        self.inception2 = Inception2()
        self.inception2 = self.load_dict(self.inception2, pretrained_dict)

        
        #self.part_predictor2 = _part_detector(nchannels[1], num_focused_parts, height // (2 ** 3),
        #                                     width // (2 ** 3), factor_of_scale_factors = 1)
        # self.foregroundAtten2 = _foregroundAtten(nchannels[1])
        # self.bilinearlayer2 = BilinearModule(nchannels[1], nchannels[1])
        # self.inception2_local = nn.ModuleList([Inception2()
        #                                        for i in range(num_focused_parts)])
        # for i in range(num_focused_parts):
        #     self.inception2_local[i] = self.load_dict(self.inception2_local[i], pretrained_dict)

        # ============== Block 3 ==============
        self.inception3 = Inception3()
        self.inception3 = self.load_dict(self.inception3, pretrained_dict)
        self.bilinearlayer2_global = BilinearModule(nchannels[2], nchannels[2])


        # self.inception3_local = nn.ModuleList([Inception3()
        #                                        for i in range(num_focused_parts)])
        # for i in range(num_focused_parts):
        #     self.inception3_local[i] = self.load_dict(self.inception3_local[i], pretrained_dict)


        # self.part_predictor3 = _part_detector(nchannels[2], num_focused_parts, height // (2 ** 3),
        #                                      width // (2 ** 3), factor_of_scale_factors = 1)

        
        # self.bilinearlayer3 = nn.ModuleList([BilinearModule(nchannels[2], nchannels[2])
        # 									for i in range(num_focused_parts)])
        # self.foregroundAtten3 = _foregroundAtten(nchannels[2])
    ...
    def forward(self, x):
        ...
        x1 = self.bilinearatt_global(x1)
        x2 = self.inception2(x1)
        # foreground_map_2 = self.foregroundAtten2(x)
        # x = x * foreground_map_2
        # x = self.bilinearlayer2(x)
        
        x3 = self.inception3(x2)  # Nx384x16x16
        #x3 = self.bilinearlayer3_global(x3)
        # theta_list1, trans_param1 = self.part_predictor1(x1)
        # theta_list2, trans_param2 = self.part_predictor2(x2)
        # theta_list3, trans_param3 = self.part_predictor3(x3)
        # #theta_list, trans_param = (theta_list1 + theta_list2 + theta_list3)/3, (trans_param1 + trans_param2 + trans_param3)/3
        # theta_list, trans_param = [sum(item) for item in zip(theta_list1, theta_list2, theta_list3)], (trans_param1 + trans_param2 + trans_param3)/3
        # theta_list = [a / 3 for a in theta_list]
        # g_loc = self.get_g_loc(x1, trans_param)

        # features_out_1_local_list = self.get_part_features(theta_list, x1)

        # for i, inception2_local_temp in enumerate(self.inception2_local):
        #     features_out_1_local_list[i] = inception2_local_temp(features_out_1_local_list[i])

        # for i, inception3_local_temp in enumerate(self.inception3_local):
        #     features_out_1_local_list[i] = inception3_local_temp(features_out_1_local_list[i])

       	# for i, bilinear_local_layer in enumerate(self.bilinearlayer3):
       	# 	features_out_1_local_list[i] = bilinear_local_layer(features_out_1_local_list[i])      

        features_global = x3  # Nx384x16x16
        #features_local = torch.cat(features_out_1_local_list, dim = 1)

        features_global = self.norm1_1(features_global)
        #features_local = self.norm1_2(features_local)
...
class BilinearBaselineAIA(nn.Module):
    def __init__(self, num_classes, loss={'xent'}, height=256, width=128, feat_dim=512,
        ...
        super(BilinearBaselineAIA, self).__init__()
        self.loss = loss
