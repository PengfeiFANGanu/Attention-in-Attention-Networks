...
class _dim_change2(nn.Sequential):
    def __init__(self, in_channels, out_channels):
        super(_dim_change2, self).__init__()
        # self.add_module('norm', nn.BatchNorm2d(in_channels))
        # self.add_module('relu1', nn.ReLU())
        # self.add_module('conv', nn.Conv2d(in_channels=in_channels, out_channels=out_channels,
        #                                   kernel_size=1, stride=1, bias=True))
...
class SoAtten(nn.Module):
    def __init__(self, in_channels):
        super(SoAtten, self).__init__()
        self.num_layers = 1
        self.hidden_size = in_channels
        # self.norm = nn.BatchNorm2d(in_channels)
        # self.relu = nn.ReLU(inplace=False)
        self.lstm = nn.LSTM(in_channels, hidden_size=in_channels, num_layers=self.num_layers, batch_first=True, bidirectional=True)
        self.conv = ConvBlock(in_channels*2, in_channels, 1)
...
class BilinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        #output = mCha * output + output   #Res setting
        output = mCha * output   #No Res setting

        output =  F.sigmoid(output) * input_x

        # m = F.avg_pool2d(x, [3,3], stride = 1, padding = 1)
        ...
        # output = F.sigmoid(x)
        # output = self.dimReduceLayer2(output)
...
class Features(nn.Module):
    def __init__(self, height=256, width=128, nchannels=(480, 832, 1024), num_focused_parts=4, drop_rate=0.1, factor_of_scale_factors=1,
        ...
        self.trans_param = self.init_trans_param(num_focused_parts)

        self.conv = Conv1()
        self.conv = self.load_dict(self.conv, pretrained_dict)
        
        #self.bilinear_global0 = BilinearAttention(192, 192)
        # ============== Block 1 ==============
        self.inception1 = Inception1()
        self.inception1 = self.load_dict(self.inception1, pretrained_dict)
        self.bilinearatt_global1 = BilinearAttention(nchannels[0], nchannels[0])
        ...
        self.part_predictor2 = _part_detector(nchannels[1], num_focused_parts, height // (2 ** 3),
                                             width // (2 ** 3), factor_of_scale_factors = 1)
        self.bilinearatt_global2 = BilinearAttention(nchannels[1], nchannels[1])
        ...
        self.bilinearatt_global3 = BilinearAttention(nchannels[2], nchannels[2])
    ...
    def get_trans_param(self, x):
        batches = x.size(0)
        trans_param_xy = self.trans_param.unsqueeze(dim=0).expand(batches, 2, self.num_parts)  # Nx2xParts
        # trans_param_xy = F.tanh(trans_param_xy)
        trans_param_scale_x = torch.ones([trans_param_xy.size(0), 1, trans_param_xy.size(2)],
                                         dtype=trans_param_xy.dtype, device=trans_param_xy.device) \
                              * 1.0
        trans_param_scale_y = torch.ones([trans_param_xy.size(0), 1, trans_param_xy.size(2)],
                              ...
                              * 0.25
        trans_param = torch.cat([trans_param_scale_x, trans_param_scale_y, trans_param_xy], dim=1)
        trans_param = trans_param.to(device=x.device)
        theta_t = trans_param[:, -2:, :]  # Nx2xParts
        # theta_t = theta_t.view(-1, 2, self.num_parts) #Nx2xParts
        # x_sample_out_list = []
        theta_list = []
        ...
        return theta_list, trans_param    

    def init_trans_param(self, num_parts):
        bias_x = torch.zeros(1, num_parts).view(-1)
        bias_y_temp = torch.linspace(start=-1, end=1, steps=2 * num_parts + 1)
        idx = torch.arange(1, num_parts + 1) * 2 - 1
        idx = idx.long()
    ...
    def forward(self, x):
        ...
        x0 = self.conv(x)
        #x0 = self.bilinear_global0(x0)
        x1 = self.inception1(x0)
        x1 = self.bilinearatt_global1(x1)
        x2 = self.inception2(x1)
        x2 = self.bilinearatt_global2(x2)
        x3 = self.inception3(x2)  # Nx384x16x16
        x3 = self.bilinearatt_global3(x3)

        theta_list, trans_param = self.get_trans_param(x1)
        ...
        return features_global, features_local
...
class IncepMBI(nn.Module):
    ...
    def forward(self, x):
        features_global, features_local = self.features(x)  # Nx384x16x16 Nx(384*Parts)x16x16
        ...
        if self.loss == {'xent'}:
            ...
            return (y_global, y_local), (f_global, f_local)
        else:
